...
const initialCourseDetails = new Map<number, CourseDetailsDto>([
  [
    1,
    {
      ...
      minAge: 16,
      maxAge: 99,
      description: 'The core general English programme covering all skills.',
  ...
  [
    2,
    {
      ...
      minAge: 18,
      maxAge: 99,
      description: 'An intensive programme for rapid progression.',
      version: versionToken,
      createdAt,
      createdByName,
      updatedAt,
  ...
  [
    3,
    {
      ...
      minAge: 12,
      maxAge: 17,
      description: 'English for young learners aged 12–17.',
      version: versionToken,
      createdAt,
      createdByName,
      updatedAt: '',
...
let courseList: CourseListItemDto[] = Array.from(initialCourseDetails.values()).map((d) => ({
  ...
  lessonsPerWeek: d.lessonsPerWeek,
  minutesPerLesson: d.minutesPerLesson,
  minAge: d.minAge,
  maxAge: d.maxAge,
...
export const courseFixtures = {
    ...
    courseList = [
      {
        ...
        minutesPerLesson: payload.minutesPerLesson,
  ...
  update: (id: number, payload: UpdateCourseRequestDto) => {
    ...
    const updated: CourseDetailsDto = { ...existing, ...payload, id, divisionName: existing.divisionName, updatedAt, updatedByName };
    courseDetails.set(id, updated);
    courseList = courseList.map((c) =>
      c.id === id ? { ...c, name: payload.name, isActive: payload.isActive, lessonsPerWeek: payload.lessonsPerWeek, minutesPerLesson: payload.minutesPerLesson, minAge: payload.minAge, maxAge: payload.maxAge, updatedAt, updatedByName } : c,
    );
    return updated;
  },
};
