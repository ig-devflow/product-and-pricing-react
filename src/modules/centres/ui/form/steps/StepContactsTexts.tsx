...
import type { CentreContactFormValue, CentreStep5Values, CentreTextContentFormValue } from '@/modules/centres/model/form.types';
...
import { createEmptyContactFormValue, createEmptyTextContentFormValue } from '@/modules/centres/model/form.mappers';
...
export const StepContactsTexts = ({ values, onChange, errors }: StepContactsTextsProps) => {
  ...
  const templatesQuery = useContentTemplatesQuery({ scope: ContentTemplateScopeDto.Centre });
  ...
  const usedContactTypes = new Set(
    contacts.map((c) => (c.contactType !== '' ? c.contactType : null)).filter((x): x is typeof CentreContactType.CentreDirector | typeof CentreContactType.DirectorOfStudies => x !== null),
  );
  ...
  return (
    <div className="centre-form__step-body">
      <AppSectionCard
        ...
        actions={
          <AppButton type="button" variant="secondary" size="sm" onClick={addContact} disabled={contacts.length >= 2}>
            {contacts.length >= 2 ? 'Both roles added' : 'Add contact'}
          </AppButton>
        }
      >
        {contacts.length === 0 ? (
          <p style={{ margin: 0, color: 'var(--color-text-disabled)' }}>No contacts yet. Use the button above to add one.</p>
        ) : (
          <div className="app-stack app-stack--lg">
            {contacts.map((c, i) => {
              const contactTypeOptions = CENTRE_CONTACT_TYPE_OPTIONS
                .filter((o) => o.value === c.contactType || !usedContactTypes.has(o.value))
              ...
              return (
                <div key={i} className="text-card">
                  <div className="text-card__header">
                    ...
                    <AppButton type="button" variant="ghost" size="sm" onClick={() => removeContact(i)}>Remove</AppButton>
                  </div>
                  <div className="app-form-grid app-form-grid--2col">
                    <AppField label="Role" required forId={`f-c-type-${i}`} error={errors[`contacts.${i}.contactType`]}>
                      <AppSelect
                        id={`f-c-type-${i}`}
                        value={c.contactType !== '' ? String(c.contactType) : ''}
                        onValueChange={(v) => updateContact(i, { contactType: v ? (Number(v) as typeof CentreContactType.CentreDirector | typeof CentreContactType.DirectorOfStudies) : '' })}
                        options={contactTypeOptions}
                        invalid={!!errors[`contacts.${i}.contactType`]}
                        placeholder="Select role"
                      />
                    </AppField>
                    <AppField label="Name" required forId={`f-c-name-${i}`} error={errors[`contacts.${i}.name`]}>
                      <AppInput
                        id={`f-c-name-${i}`}
                        value={c.name}
                    ...
                    <AppField label="Email" forId={`f-c-email-${i}`} error={errors[`contacts.${i}.email`]}>
                      <AppInput
                        id={`f-c-email-${i}`}
                        type="email"
                        value={c.email}
                    ...
                    <AppField label="Signature image" error={errors[`contacts.${i}.signatureImage`]} hint="PNG, JPEG, WebP, or SVG up to 2 MB.">
                      <CentreImageUpload
      ...
      <AppSectionCard
        ...
        actions={<AppButton type="button" variant="secondary" size="sm" onClick={addText}>Add text block</AppButton>}
      >
        {texts.length === 0 ? (
          <p style={{ margin: 0, color: 'var(--color-text-disabled)' }}>No text content yet. Use the button above to add a block.</p>
        ) : (
          <div className="app-stack app-stack--lg">
            {texts.map((t, i) => {
              const hasContent = !!t.content.trim();
              const over = t.content.length > 10000;
              const templateName = templates.find((tpl) => String(tpl.id) === t.contentTemplateId)?.name || 'Untitled text block';
                ...
                <div key={i} className="text-card">
                  <div className="text-card__header">
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <span className={`text-card__chars${over ? ' text-card__chars--over' : ''}`}>{t.content.length} / 10,000</span>
                      <AppButton type="button" variant="ghost" size="sm" onClick={() => removeText(i)}>Remove</AppButton>
                    </div>
                  </div>
                  <div className="app-form-grid app-form-grid--2col">
                    <AppField label="Template" required forId={`f-t-tpl-${i}`} error={errors[`texts.${i}.contentTemplateId`]}>
                      <AppSelect
                        id={`f-t-tpl-${i}`}
                    ...
                    <AppField label="Audience" forId={`f-t-aud-${i}`} hint="Leave empty to apply to all audiences.">
                      <AppSelect
                        id={`f-t-aud-${i}`}
                        value={t.audienceId}
                    ...
                    {hasContent ? (
                      <AppField label="Format" required forId={`f-t-fmt-${i}`} error={errors[`texts.${i}.format`]}>
                        <AppSelect
                          id={`f-t-fmt-${i}`}
                          value={t.format === ContentFormat.None ? ContentFormat.PlainText : t.format}
                          onValueChange={(v) => updateText(i, { format: (v ?? ContentFormat.PlainText) as ContentFormat })}
                      ...
                      <div className="app-field">
                        <span className="app-label" style={{ color: 'var(--color-text-tertiary)' }}>Format</span>
                        <p className="app-field__hint" style={{ padding: '14px 16px', border: '1px dashed var(--color-border-strong)', borderRadius: 'var(--radius-xs)', background: 'var(--color-bg-surface-soft)', minHeight: 'var(--control-height-md)', display: 'flex', alignItems: 'center', margin: 0 }}>
                  ...
                  <AppField label="Content" forId={`f-t-cnt-${i}`} error={errors[`texts.${i}.content`]}>
                    <AppTextarea
                      id={`f-t-cnt-${i}`}
