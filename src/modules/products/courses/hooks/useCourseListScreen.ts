...
export const useCourseListScreen = () => {
  ...
  const query = useCourseListQuery(divisionId, {
    search: submittedSearch.trim() || undefined,
    isActive: activeOnly || undefined,
    page,
    pageSize,
  });
  const errorMessage = getApiErrorMessage(query.error, 'Failed to load courses.');
  ...
  const emptyMessage = useMemo(() => {
    ...
    return term ? `No courses matched "${term}".` : 'Try changing the search or create a new course.';
  }, [submittedSearch]);

  return {
    pageHeader: coursePageHeaders.list,
    searchTerm,
    setSearchTerm,
    activeOnly,
    setActiveOnly,
    ...
    handleCreateClick: () => navigate(PRODUCT_MANAGER_ROUTES.courses.create),
  };
};
