...
export const CategorisationSection = ({ divisionId }: CategorisationSectionProps) => {
  const { control, formState: { errors } } = useFormContext<PackageFormValues>();
  ...
  return (
    ...
    >
      <AppFormGrid>
        <AppField label="Product category" forId="pkg-product-cat" error={errors.productCategoryId?.message} required>
          {({ labelId, describedBy }) => (
            <Controller
        ...
        <AppField label="Account category" forId="pkg-account-cat" error={errors.accountCategoryId?.message} required>
          {({ labelId, describedBy }) => (
            <Controller
