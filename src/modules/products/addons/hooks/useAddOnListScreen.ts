...
import { useAddOnListQuery } from '../queries/useAddOnListQuery';
...
export const useAddOnListScreen = () => {
  ...
  const errorMessage = getApiErrorMessage(query.error, 'Failed to load add-ons.');
  ...
  const emptyMessage = useMemo(() => {
    ...
    return term ? `No add-ons matched "${term}".` : 'Try changing the search or create a new add-on.';
  }, [submittedSearch]);

  return {
    pageHeader: addonPageHeaders.list,
    searchTerm,
    setSearchTerm,
    activeOnly,
    setActiveOnly,
    ...
    handleCreateClick: () => navigate(PRODUCT_MANAGER_ROUTES.addons.create),
  };
};
