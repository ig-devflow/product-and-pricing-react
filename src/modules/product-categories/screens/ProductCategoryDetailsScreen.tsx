...
export const ProductCategoryDetailsScreen = () => {
  ...
  if (query.isLoading) {
    return (
      <section className="app-page">
        <AppPageHeader eyebrow="Pricing Reference Data" title="Product category" />
  ...
  return (
    <section className="app-page">
      <AppPageHeader
        ...
        title={category.name}
        subtitle={`Product category · ID ${category.id}`}
        actions={
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <AppButton
              type="button"
              variant="secondary"
              onClick={() => navigate(CATEGORIES_MANAGER_ROUTES.productCategories.list)}
            >
              Back to list
            </AppButton>
            <AppButton
              type="button"
              variant="primary"
              onClick={() => navigate(CATEGORIES_MANAGER_ROUTES.productCategories.edit(category.id))}
            >
              Edit
            </AppButton>
          </div>
