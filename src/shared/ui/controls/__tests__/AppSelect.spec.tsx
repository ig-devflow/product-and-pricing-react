...
describe('AppSelect', () => {
  ...
  it('filters options in searchable mode and selects a filtered value', async () => {
    const onValueChange = vi.fn()
    const { user, combobox } = setupSelect({
      searchable: true,
    ...
    await user.click(combobox)
    const searchInput = screen.getByRole('textbox', { name: 'Filter options' })
    await user.type(searchInput, 'mark')
    ...
    expect(onValueChange).toHaveBeenCalledWith('markdown')
    expect(combobox).toHaveTextContent('Markdown')
  })

  it('shows empty state text in searchable mode when no options match', async () => {
    const { user, combobox } = setupSelect({
      searchable: true,
      noOptionsText: 'No matching formats',
    })
